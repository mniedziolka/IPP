...
/** @brief Szuka miasta.
 * Przeszukuje mapę w celu znalezienia konkretnego miasta.
 * @param[in,out] map – wskaźnik na mapę;
 * @param[in,out] city – wskaźnik na nazwę miasta;
 * @return Zwraca wskaźnik na szukane miasto, lub NULL
 * jesli miasto nie istnieje.
 */
City* findCity(Map* map, const char *city);

/** @brief Szuka droki krajowej.
 * Przeszukuje mapę w celu znalezienia drogi krajowej.
 * @param[in,out] map – wskaźnik na mapę;
 * @param[in] routeId – id drogi krajowej;
 * @return Zwraca wskaźnik na szukaną drogę krajową,
 * lub NULL jesli droga krajowa o tym id nie istnieje.
 */
 ...
 */
bool repairRoad(Map *map, const char *city1, const char *city2, int32_t repairYear);
...
/** @brief Tworzy ustaloną drogę krajową.
 * Tworzy drogę krajową przechodzącą przez podane miasta.
 * Miasta i krawędzie między nimi muszą już istnieć.
 * @param[in,out] map               – wskaźnik na strukturę przechowującą mapę dróg;
 * @param[in] routeId               – numer drogi krajowej;
 * @param[in,out] citiesOnRoute     – wskaźnik na początek listy zawierającej miasta;
 * @return Wartość @p true, jeśli droga krajowa została utworzona.
 * Wartość @p false, jeśli wystąpił błąd: któryś z parametrów ma niepoprawną
 * wartość, istnieje już droga krajowa o podanym numerze.
 */
 ...
 * Usuwa drogę krajową ze wszystkich odcinków dróg i z tablicy dróg krajowych.
 * @param[in,out] map    – wskaźnik na strukturę przechowującą mapę dróg;
 * @param[in] routeId    – numer drogi krajowej.
 * @return Wartość @p true jeśli droga krajowa została usunięta.
 * Wartość @p false, jeśli parametry są niepoprawne.
 */
